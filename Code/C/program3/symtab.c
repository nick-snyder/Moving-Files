...
const char *symtabNext(void *iteratorHandle, void **returnData)
{
    ...
    // initialize a local variable to contain the pointer currently stored 
    // in the symbol member of the linked struct pointed to by the iterator struct
    char *copySymbol = iterator->node->symbol;

    // return the data member of that linked struct through the returnData parameter, 
