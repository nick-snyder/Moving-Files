...
int symtabInstall(void *symtabHandle, const char *symbol, void *data)
{
    ...
    else
    {
        ...
        char *copySymbol = malloc(sizeof(char[symbolLength]) + 1);
        if (copySymbol == NULL)
        {
            return 0;
        }
        ...
        //free(copySymbol);     // cause more errors
        //free(newNode->data);
        //free(newNode);
    }
...
int symtabEditData(void *symtabHandle, const char *symbol, void *newData)
{
    Table *table = symtabHandle;
    
    // go to symbol in table
    Node *node = internalLookup(table, symbol);
...
{
    // // The symtabHandle parameter is actually a pointer to a control struct so assign it to a local variable of this type.
    // void *localSymtabHandle = symtabHandle;
    ...
    // while (symtabLookup(symtabHandle, symtabHandle->array[0]))
    // {
...
const char *symtabNext(void *iteratorHandle, void **returnData)
{
    ...
    //     char *copySymbol = iteratorHandle->node->symbol;
    ...
    //     // update the iterator struct members accordingly. 
    //     // (Note that the index member might not change, if there is another symbol at the current level in the table that has not yet been returned.)


    //     //  If all symbols in the table have now been processed
    //     if (1 == 1)
    //     {
    //         // set the linked struct pointer member of the iterator struct to NULL.
    //         // (Note that this would happen when your search for the next symbol to return hits the end of the table without finding a symbol.)
    //         iteratorHandle->node = NULL;
    //     }
