...
void putConcurrentBuffer(void *handle, void *p)
{
    ...
    if (pthread_mutex_lock(&buffer->mu) != 0)
    {
        exit(-1);
    }

    // printf("buffer->size = %d\n", buffer->size);    // <----- ERROR HERE. buffer->size = NULL. Maybe resolved?
    ...
    while (buffer->size == buffer->capacity)
    {
        ...
        if (pthread_cond_wait(&buffer->cv, &buffer->mu) != 0)
        {
            exit(-1);
        } 
    }
    ...
    if (pthread_cond_signal(&buffer->cv) != 0)
    {
        exit(-1);
    }

    // unlock buffer when done
    if (pthread_mutex_unlock(&buffer->mu) != 0)
    {
        exit(-1);
    }
}
